...
Font::Font(const char* const a_FileLocation, void* a_manager) {
	m_manager = a_manager;
...
void Font::loadTextures() {
	...
	for (unsigned int i = 0; i < m_Pages.size(); i++) {
		...
		GLuint* textureID = &m_Pages[i].m_texture.id;
		...
		m_Pages[i].m_texture.filepath = m_FileName;
		m_Pages[i].m_texture = ResourceManager::getTexture(filename, m_Pages[i].m_texture.width, m_Pages[i].m_texture.height, bpp, m_manager);
...
void Font::genVertex() {
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 6 * 4, NULL, GL_STATIC_DRAW); // need static?
...
Font::~Font() {
	for (unsigned int i = 0; i < m_Pages.size(); i++) {
		glDeleteTextures(1, &m_Pages[i].m_texture.id);
		m_Pages[i].m_texture.id = 0;
	}
...
void Font::draw(const char* const a_Letters, int const a_X, int const a_Y, Shader* shaderProg, float const a_Scale, glm::vec3 a_Color) {

	shaderProg->setUniform("Color", a_Color.x, a_Color.y, a_Color.z);

	glActiveTexture(GL_TEXTURE0);
	shaderProg->setUniform("image", 0);
	...
	while (a_Letters[index] != '\0') {
		...
		if (iteratorLetter != m_Chars.end()) {
			...
			/*
			glBindTexture(GL_TEXTURE_2D, m_Pages[letter.page].m_texture.id);
			//glBindBuffer(GL_ARRAY_BUFFER, VBO);
